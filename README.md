...
SmartClip now uses a premium dark, portrait-first mobile shell with Home, Projects, Downloads, and Settings destinations. Device and URL workflows remain deliberately isolated, while lightweight history, known-session downloads, and non-sensitive defaults are persisted locally. See [the UI architecture and design system](docs/ui-architecture.md) for navigation, tokens, privacy, accessibility, responsive behavior, and known limitations.

SmartClip now also has an isolated, personal-use **URL processing service** in `backend/`. It accepts public yt-dlp-compatible URLs, runs deterministic (non-AI) highlight analysis, and temporarily serves completed vertical MP4 candidates. Android local videos remain entirely on-device; the APK is not connected to this API yet. See [the URL backend guide](docs/url-backend.md) for its authenticated API, Docker/Render deployment, limits, cleanup, security model, and examples.
...
- **Auto, 720p, and 1080p:** Auto chooses 1080×1920 only for sufficiently detailed sources/capable devices; otherwise it uses 720×1280. Explicit upscaling displays a warning.
...
- `frontend/plugins/local-video-picker/` opens Android's picker, reads metadata, trims with platform `MediaExtractor`/`MediaMuxer`, and publishes with `MediaStore`.
...
- `backend/` contains the isolated FastAPI URL-processing service. It is not used for local Android videos and is not yet integrated into the APK.
...
The Android build requests no internet, broad storage, camera, or microphone permissions.

## Manual trim and gallery export

The editor validates a minimum one-second range and provides 10-, 15-, 30-second and full-duration presets. The native plugin uses Android's maintained platform `MediaExtractor` and `MediaMuxer` APIs—no external media library was added. H.264 video and optional AAC audio samples are copied without decoding, preserving encoded resolution and the source rotation hint. The start seeks to the next sync sample, so it can be slightly later than requested. Unsupported codecs are reported rather than silently re-encoded.

Scoped `MediaStore` publishes MP4 output under **`Movies/SmartClip`**, named like `SmartClip_20260724_211500.mp4`. The UI shows actual duration, size, Open and Share actions, real sample/byte progress, and safe cancellation. Android 10+ is required for permission-free gallery export. No permissions were added: picker URI grants and app-owned MediaStore output avoid `READ_MEDIA_VIDEO`, legacy storage, and all-files access. Background work continues while the process lives; process death cancels it and cleanup is best-effort.
