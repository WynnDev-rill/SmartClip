...
it("removes an uploaded video",async()=>{render(<VideoUpload/>);fireEvent.change(screen.getByLabelText(/choose/i),{target:{files:[new File(["x"],"clip.mp4")]}});await userEvent.click(await screen.findByRole("button",{name:/remove video/i}));await waitFor(()=>expect(mocks.deleteVideo).toHaveBeenCalledWith("abc"));expect(screen.getByText("Drop your video here")).toBeInTheDocument()})});
